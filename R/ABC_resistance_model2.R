...
res[1] <- mean(round(pairwise.fstb(gi),3))

for (i in 2:5000)
## a single run...
{ 
#run for 100 generations
simpops <- PopGenReport::run.popgensim(simpops, steps=1, cost.mat, n.offspring=para$n.offspring, n.ind=para$n.ind, para$mig.rate, para$disp.max, para$disp.rate,  para$n.allels, para$mut.rate, n.cov=para$n.cov, rec="none")
#convert to genind object (smaller)
gi <- PopGenReport::pops2genind(simpops)
res[i] <- mean(round(pairwise.fstb(gi),3))
